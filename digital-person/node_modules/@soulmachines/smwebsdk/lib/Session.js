...
var chroma_key_worker_1 = require("./dedicated-workers/chroma-key.worker");
...
var Session = /** @class */ (function () {
    function Session(videoElement, serverUri, connectUserText, accessToken, audioOnly, requestedUserMedia, requiredUserMedia, echoCancellationEnabled, logger, connectionState, chromaKeyOptions) {
        var _this = this;
        this._chromaKeyedCanvas = null;
        this._videoElementStyleObserver = null;
    ...
    Session.prototype.setVideoElement = function (videoElement) {
        var _this = this;
        var _a, _b;
        ...
        if (this._videoElement) {
            this._videoElement.removeEventListener('loadeddata', this.onVideoLoaded.bind(this));
            this._removeListeners = this._removeListeners.filter(function (removeLister) { return removeLister.target !== _this._videoElement; });
            if (videoElement) {
                videoElement.srcObject = this._videoElement.srcObject;
            }
        }
        (_a = this._videoElementStyleObserver) === null || _a === void 0 ? void 0 : _a.disconnect();
        this._videoElement = videoElement;
        this.setupVideoElement();
        if ((_b = this._chromaKeyOptions) === null || _b === void 0 ? void 0 : _b.enabled)
            this.restoreChromaKeySettings();
    };
    Session.prototype.restoreChromaKeySettings = function () {
        var _a, _b;
        if (!this._videoElement)
            return;
        ...
        if (this._videoElement.srcObject) {
            ...
            if (!(this.supportsInsertableStreams && this.supportsTransferableStreams) &&
                ...
                (_a = this._chromaKeyedCanvas.parentElement) === null || _a === void 0 ? void 0 : _a.removeChild(this._chromaKeyedCanvas);
                (_b = this._videoElement.parentElement) === null || _b === void 0 ? void 0 : _b.appendChild(this._chromaKeyedCanvas);
                ...
                if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
                    ...
                    this._videoElement.requestVideoFrameCallback(this.processFrameForChromaKey.bind(this));
                }
    ...
    Object.defineProperty(Session.prototype, "chromaKeyedCanvas", {
        get: function () {
    ...
    Session.prototype.setupVideoElement = function () {
        if (this._videoElement) {
    ...
    Session.prototype.copyStyleFromVideoElementToCanvas = function () {
        if (this._chromaKeyedCanvas && this._videoElement) {
            var videoElementStyles_1 = getComputedStyle(this._videoElement);
            var cssText = videoElementStyles_1.cssText;
            if (!cssText) {
                cssText = Array.from(videoElementStyles_1).reduce(function (cssString, property) {
                    var propertyValue;
                    if (property === 'display') {
                        // don't copy display:none style because video element is hidden on purpose
                        propertyValue = videoElementStyles_1
                            .getPropertyValue(property)
                            .replace('none', '');
                    }
                    else {
                        propertyValue = videoElementStyles_1.getPropertyValue(property);
                    }
                    return "".concat(cssString).concat(property, ":").concat(propertyValue, ";");
                }, '');
                this._chromaKeyedCanvas.style.cssText = cssText;
            }
        }
    };
    Session.prototype.observeAndCopyStyleChangesFromVideoElementToCanvas = function () {
        var _this = this;
        if (!this._videoElementStyleObserver) {
            this._videoElementStyleObserver = new MutationObserver(function (mutationList, observer) {
                _this.copyStyleFromVideoElementToCanvas();
            });
        }
        this._videoElementStyleObserver.observe(this._videoElement, {
            attributes: true,
        });
        this._videoElement.addEventListener('resize', this.copyStyleFromVideoElementToCanvas.bind(this));
    };
    Session.prototype.processFrameForChromaKey = function (now, metadata) {
        if (this._chromaKeyedCanvas) {
            this._chromaKeyedCanvas.width = metadata.width;
            this._chromaKeyedCanvas.height = metadata.height;
            (0, chroma_key_worker_1.render_rgb)(this._chromaKeyedCanvas.width, this._chromaKeyedCanvas.height, this._videoElement);
            this._videoElement.requestVideoFrameCallback(this.processFrameForChromaKey.bind(this));
        }
    };
    Session.prototype.onConnectedSuccess = function () {
        ...
        if (((_a = this._chromaKeyOptions) === null || _a === void 0 ? void 0 : _a.enabled) && this._videoElement) {
            this.setupChromaKeyedVideo();
        }
        ...
        (_b = this._onMicrophoneActive) === null || _b === void 0 ? void 0 : _b.call(this.isMicrophoneActive());
        (_c = this._onCameraActive) === null || _c === void 0 ? void 0 : _c.call(this.isCameraActive());
    };
    Session.prototype.hideVideoElement = function (videoElement) {
        videoElement.hidden = true;
        videoElement.style.display = 'none';
    };
    Session.prototype.setupChromaKeyedVideo = function () {
        var _a;
        if (this._videoElement && ((_a = this._chromaKeyOptions) === null || _a === void 0 ? void 0 : _a.enabled)) {
            if (this.supportsInsertableStreams && this.supportsTransferableStreams) {
    ...
    Session.prototype.setupChromaKeyedCanvas = function (chromaKeyOptions) {
        var _a;
        this._chromaKeyedCanvas = document.createElement('canvas');
        this.copyStyleFromVideoElementToCanvas();
        (_a = this._videoElement.parentElement) === null || _a === void 0 ? void 0 : _a.appendChild(this._chromaKeyedCanvas);
        this.hideVideoElement(this._videoElement);
        this.observeAndCopyStyleChangesFromVideoElementToCanvas();
        (0, chroma_key_worker_1.updateChromaKeyOptions)(chromaKeyOptions);
        (0, chroma_key_worker_1.initWebGL_rgb_with_canvas)(this._chromaKeyedCanvas);
    };
    Session.prototype.setupTransferableStreamChromaKeyedVideo = function () {
        var worker = new Worker('/dedicated-workers/chroma-key.worker.js', {
            type: 'module',
        });
        ...
        if (window.RTCRtpScriptTransform) {
            ...
            var transceiver = this._peerConnection.getTransceivers()[0];
            transceiver.receiver.transform = new window.RTCRtpScriptTransform(worker, { name: 'receiverTransform' });
        }
        else {
            var videoTrack = this.getTrackByKind(this._remoteStream, 'video');
            var audioTrack = this.getTrackByKind(this._remoteStream, 'audio');
            ...
            var mediaStream = new MediaStream([generator]);
    ...
    Session.prototype.setupRequestVideoFrameCallbackChromaKeyedVideo = function () {
        this._videoElement.requestVideoFrameCallback(this.processFrameForChromaKey.bind(this));
    };
    Session.prototype.setupRequestAnimationFrameChromaKeyedVideo = function () {
        var _this = this;
        var update = function () {
            if (_this._chromaKeyedCanvas) {
                _this._chromaKeyedCanvas.width = _this._videoElement.videoWidth;
                _this._chromaKeyedCanvas.height = _this._videoElement.videoHeight;
                (0, chroma_key_worker_1.render_rgb)(_this._chromaKeyedCanvas.width, _this._chromaKeyedCanvas.height, _this._videoElement);
                setTimeout(function () {
    ...
    Object.defineProperty(Session.prototype, "supportsEncodedTransform", {
        /**
    ...
    Object.defineProperty(Session.prototype, "supportsInsertableStreams", {
        /**
    ...
    Object.defineProperty(Session.prototype, "supportsTransferableStreams", {
        /**
